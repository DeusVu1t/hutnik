...
<div class="single-post-temp">
    <div class="container">
        ...
        <div class="post-container">
            ...
            <div class="post-header">
              <?php
                    $categories = get_the_category(); // Получаем категории текущей записи
                    if (!empty($categories)) {
                        $main_category = $categories[0]->name; // Берем первую категорию
                        $main_category_slug = $categories[0]->slug; // Получаем slug (часть URL) первой категории
                        $post_link = esc_url(get_site_url()) . '/aktualnosci?category=' . $main_category_slug;

                        $is_current_category = isset($_GET['category']) && $main_category_slug === $_GET['category'] ? 'active' : '';

                        echo '<p class="post-categ ' . $is_current_category . '"><a class="subhead" href="' . $post_link . '">' . esc_html($main_category) . '</a></p>';
                    }
                    ?>
	...
	h-162.6L569.16,687.85V687.83z" />
        ...
        <div class="gallery-grid">
            <?php if (have_rows('galereja')) : while (have_rows('galereja')) : the_row();  ?>

                    <div class="single-image">
                        <?php $obrazek = get_sub_field('obrazek'); ?>
                        <?php if ($obrazek) : ?>
                            <a class="example-image-link" data-lightbox="example-set" href="<?php echo esc_url($obrazek['url']); ?>" data-title="Click the right half of the image to move forward.">
                                <?php
                                $image_url = $obrazek['sizes']['medium']; // Получаем URL изображения размера "medium"
                                $image_alt = $obrazek['alt'];
                                ?>
